...
""")
            ...
            if "<version>0.4.18</version>" not in (app_dir / "appinfo/info.xml").read_text():
                raise AssertionError("upgrade app is not version 0.4.18")
            occ("upgrade")
...
WHERE k.key_id = 'default' AND k.binding_id = 'upgrade-current'
  AND k.source_hash = b.source_hash AND b.retired_at = 0;""")
            ...
            print(f"{case}: direct 0.4.6→0.4.18 upgrade tombstone smoke passed")
        finally:
