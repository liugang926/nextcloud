...
The local implementation includes change hints with periodic full reconciliation and two-scan deletion confirmation. The WeKnora patch applies a live, fail-closed source and ETag guard to tested search, chat, direct file, history, resource, embed, MCP, and derived-content paths. A minimal durable version row stages an invisible candidate and publishes it after parsing, while keeping older rows for later recovery policy. Complete revision history and garbage collection, durable event delivery and acknowledgement, two-phase binding policy, an enterprise AD permission matrix, load/fault testing, and actual coordinated backup recovery remain open. Keep this stack on synthetic local data until the [V1 status and remaining work](docs/v1-status.md) are addressed.
