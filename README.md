...
The local implementation includes change hints with periodic full reconciliation and two-scan deletion confirmation. The WeKnora patch applies a live, fail-closed source and ETag guard to tested search, chat, direct file, history, resource, embed, MCP, and derived-content paths. A durable version row stages an invisible candidate and publishes it after parsing and an exact signed source recheck, while keeping older rows for later recovery policy. The WeKnora patch now also appends each future local version-row mutation to a per-source observation ledger; migrated rows keep an explicit unknown-history flag, so this is not a complete copy inventory or the PRD document-revision lifecycle. The patch has a signed durable event inbox and a leased dispatcher that queues incremental source reconciliation with a complete manifest preflight, targeted content refresh, and an independent daily full-content audit; Nextcloud signs and retries scoped delivery. Nextcloud 0.4.22 and the WeKnora patch provide a durable, headless two-phase source-pairing workflow for a new empty dedicated knowledge base. [Local operations](scripts/ops/README.md) include operator CLIs for initial pairing, pending abort, active key rotation, and retry. A paired synthetic source passed ingestion, rename, signed event application and two-scan deletion probes. Receipt, queue admission and application remain separate states; Nextcloud retains connected hints until a signed applied watermark covers them. Binding-wide Stop publication blocks new source reads but does not prove WeKnora index cleanup. Ordinary binding deletion still rejects pairing history. An exact, signed retirement protocol closes only a newly paired source that has never admitted sync, events, or content. The WeKnora PostgreSQL patch also provides a first-stage withdrawal for an indexed pair: it pauses publication, revokes its event receiver and records observed local copies, but never asserts a complete inventory or sends a retirement ACK. A fresh disposable dual-stack HTTP drill verified this boundary with one indexed synthetic file. Durable GC now deletes eligible tenant-scoped local source objects with a confirmed-byte count; derived indexes, cloud objects and history still need cleanup. An isolated same-directory synthetic LDAP two-user authorization and revocation drill passed; enterprise AD acceptance, complete indexed-source decommission and physical cleanup, production permissions, load/fault testing, and actual coordinated backup recovery remain open. The connector streams one downloaded file at a time after validating the complete metadata manifest and skips unchanged content for scoped hints, but still buffers the manifest and one file body, has no mid-scan resume, and has not passed the 10,000-file / 100-GB pilot. The local Nextcloud sender worker and WeKnora dispatcher now each run a five-second cycle, but end-to-end P95 event-to-task latency has not been measured or accepted against the PRD's 10-second target. Keep this stack on synthetic local data until the [V1 status and remaining work](docs/v1-status.md) are addressed.
