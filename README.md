...
To allow devices on the same LAN to reach this development stack after bootstrap, run `./scripts/allow-lan-access.sh` (or pass the host's private IPv4 address). It binds the Nextcloud HTTP port to all host interfaces, adds that address to Nextcloud's trusted domains, and prints the LAN URL. If the host IP changes, run the script again. This development endpoint uses HTTP and the generated test credentials.
