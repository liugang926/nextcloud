...
This local slice proves app installation, a bound folder's paginated manifest and content API, persistent manual withdrawal exclusion, and the connector protocol with two-scan deletion confirmation. It does not yet implement persistent event delivery, safe version publication, source permission checks for each WeKnora user, or full retrieval and chat authorization. Do not use the connector for enterprise documents until those controls are implemented and tested. A full knowledge ingestion and Q&A check also needs a test embedding/chat model configured in the local WeKnora instance. See [the V1 status and authorization gaps](docs/v1-status.md).
