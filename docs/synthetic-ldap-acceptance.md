...
For a generated, disposable Compose stack and an automated fresh nested-group HTTP matrix, use [synthetic-ldap-compose.md](synthetic-ldap-compose.md).
...
The fixture models GUID/SID and primary-group-shaped attributes, but OpenLDAP does not establish enterprise AD behavior for nested groups, primary group resolution, disabled-account propagation, Kerberos/SSO, or production folder ACLs. The recorded cross-system drill covered the baseline and `group_removed` cases and old JWT direct read paths. The later generated fixture in [synthetic-ldap-compose.md](synthetic-ldap-compose.md) passed a fresh cross-system `nested_group` six-field HTTP matrix. Its primary-only LDAP/WeKnora topology passed but Nextcloud `user_ldap` did not grant the file, so `primary_group` remains failed. The identity-conflict probe and revocation watcher have offline contracts; a fresh LDAP-backed Team folder run has **not** been recorded. The patched WeKnora LDAP adapter's separate network integration suite passed its binary GUID/SID, primary/nested, disabled-account, LDAPS/StartTLS and failure-path tests in a disposable OpenLDAP project on 2026-09-24. It did not establish the full Q&A citation flow or every alternate retrieval entry point listed in [AD-acceptance.md](../scripts/ops/AD-acceptance.md). Run those independently before marking the broader AD acceptance complete.
